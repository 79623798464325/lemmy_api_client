...
///
/// **Note:** This is a placeholder for the upcoming Lemmy V4 API. Endpoints
/// will be implemented as the V4 API specification is finalized.
class LemmyApiV4 implements LemmyApi {
  ...
  Uri _buildUri(String path, [Map<String, String>? queryParams]) {
    // Paths starting with '!' are absolute (bypass extraPath prefix)
    final resolvedPath = path.startsWith('!') ? path.substring(1) : '$extraPath$path';
    return _isLocalhost ? Uri.http(host, resolvedPath, queryParams) : Uri.https(host, resolvedPath, queryParams);
  }
  ...
  Future<http.Response> _makeRequest(LemmyApiQuery query, Map<String, dynamic> queryJson) {
    switch (query.httpMethod) {
      ...
      case HttpMethod.delete:
        return _client.delete(_buildUri(query.path), body: jsonEncode(queryJson), headers: _buildHeaders(queryJson, includeContentType: true));
    }
  }
